...
public static class ModelBuilderExtensions
{
    ...
    public static void ApplyClientSideIdGeneration(this ModelBuilder modelBuilder)
    {
        ...
        {
            // 2. Check if the current entity inherits from JtBaseDomainEntity
            if (typeof(JtBaseDomainEntity).IsAssignableFrom(entityType.ClrType))
                // 3. Configure the Id property to never generate values from the DB
                modelBuilder.Entity(entityType.ClrType)
                    .Property(nameof(JtBaseDomainEntity.Id))
                    .ValueGeneratedNever();
